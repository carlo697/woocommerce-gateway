...
class ProductoWoocommerce implements ShouldQueue
{
    ...
    {
        $producto = Product::where('status', 'to_process')->with('productStore')->limit(20)->get();

        error_log($producto);

        // $this->actualizar_woo($producto);
    }
    public function actualizar_woo($producto)
    {

        $resultado = DB::table('wplp_postmeta')->where('meta_value', $producto->sku)->first();
        if (!$resultado) {
